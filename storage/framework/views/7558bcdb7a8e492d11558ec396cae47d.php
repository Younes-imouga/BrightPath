...
<body class="bg-gray-100 text-gray-800 flex flex-col min-h-screen">
  <main class="container mx-auto p-4 flex-grow">
    <section class="my-8">
      ...
      <form action="<?php echo e(route('admin.updateQuiz', $quiz->id)); ?>" method="POST" class="bg-white rounded-lg shadow p-8">
        <?php echo csrf_field(); ?>
        <?php echo method_field('PUT'); ?>
        <div class="mb-4">
          <label for="name" class="block text-gray-700">Quiz Name</label>
          <input type="text" name="name" id="name" value="<?php echo e($quiz->name); ?>" required class="w-full p-3 border rounded" />
        </div>
        <div class="mb-4">
          ...
          <select name="course_id" id="course_id" class="w-full p-3 border rounded" required>
            ...
            <?php $__currentLoopData = $courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $course): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <option value="<?php echo e($course->id); ?>" <?php echo e($quiz->course_id == $course->id ? 'selected' : ''); ?>><?php echo e($course->name); ?></option>
